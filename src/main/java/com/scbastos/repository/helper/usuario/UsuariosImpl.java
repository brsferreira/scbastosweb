...
import com.scbastos.model.UsuarioGrupo;
...
public class UsuariosImpl implements UsuariosQueries {
	...
	public Optional<Usuario> porEmailEAtivo(String email) {
		...
		@Override   //VERIFICAR O TIPO DE RETORNO NO CASO DE UMA STRING
		public List<Usuario> CorretorAtivo(){
			return (List<Usuario>) manager.createQuery(
					"SELECT u FROM Usuario u JOIN u.grupos g WHERE g.id_grupo = 2 AND u.ativo = true", Usuario.class)
					.getResultList();
		}
		...
		private void adicionarFiltro(UsuarioFilter filtro, Criteria criteria) {
			...
			if (filtro != null) {
				...
				if (filtro.getGrupos() != null && !filtro.getGrupos().isEmpty()) {
					...
					for (Long codigoGrupo : filtro.getGrupos().stream().mapToLong(Grupo::getId_grupo).toArray()) {
						
						System.out.println("codigoGrupo: " + codigoGrupo);
						
						DetachedCriteria dc = DetachedCriteria.forClass(UsuarioGrupo.class);
						dc.add(Restrictions.eq("id.grupo.id_grupo", codigoGrupo));
						dc.setProjection(Projections.property("id.usuario"));
						subqueries.add(Subqueries.propertyIn("id_usuario", dc));
					}
